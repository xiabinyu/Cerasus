...
void logIMUData(void)
{
  ...
  if (enableAccel) // If accelerometer logging is enabled
  {
    ...
    {
      imuLog += String(imu.calcAccel(imu.ax)*9.7964) + ", ";
      imuLog += String(imu.calcAccel(imu.ay)*9.7964) + ", ";
      imuLog += String(imu.calcAccel(imu.az)*9.7964) + ", ";
    }
    else
    {
  ...
  if (enableGyro) // If gyroscope logging is enabled
  {
    if ( enableCalculatedValues ) // If in calculated mode
    {
      imuLog += String(imu.calcGyro(imu.gx)/57.3) + ", ";
      imuLog += String(imu.calcGyro(imu.gy)/57.3) + ", ";
      imuLog += String(imu.calcGyro(imu.gz)/57.3) + ", ";
    }
    else
    {
